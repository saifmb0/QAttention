...
def ragged_eagle_context(branching_factor: int, max_depth: int,
    ...
    def _ragged_attn_forward(
        ...
        if past_key_value is not None:
            _pk, _pv = past_key_value[0], past_key_value[1]
            # EAGLE's SinkCache has a custom .cat() method; plain tensors (e.g.
            # when the KV cache is a HuggingFace DynamicCache after BnB/GPTQ
            # loading) do not.  Handle both.
            if isinstance(_pk, torch.Tensor):
                key_states   = torch.cat([_pk, key_states],   dim=2)
                value_states = torch.cat([_pv, value_states], dim=2)
            else:
                key_states   = _pk.cat(key_states,   dim=2)
                value_states = _pv.cat(value_states, dim=2)
        past_key_value = None
...
def main() -> None:
    ...
    parser.add_argument("--csv-name", default="w4a16_e2e.csv")
